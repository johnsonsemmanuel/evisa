...
import { PASSPORT_AUTHORITIES, getAuthorityByCountry } from "@/lib/passport-authorities";
...
function validateEmail(email: string): boolean {
  ...
  const suspiciousDomains = ["gmial.com", "gmali.com", "gmal.com", "yaho.com", "hotmal.com", "outlok.com"];
...
function NewApplicationPageInner() {
  ...
  const [passportVerificationResult, setPassportVerificationResult] = useState<any>(null);
  ...
  const defaultForm: Record<string, string> = {
    ...
    passport_number: "", passport_issue_date: "", passport_expiry: "", passport_issuing_authority: "",
  ...
  const handleNationalityChange = useCallback((nationality: string) => {
    ...
    // Auto-populate issuing authority based on nationality
    const authority = getAuthorityByCountry(nationality);
    if (authority) {
      set("passport_issuing_authority", authority);
    }
    
    setPassportVerificationResult(null);
  }, [set]);
    ...
    api.get(`/applicant/applications/${resumeAppId}`)
      .then(res => {
        ...
        setForm(prev => ({
          ...
          passport_issuing_authority: app.passport_issuing_authority || prev.passport_issuing_authority,
  ...
  const handleCreateDraft = async () => {
    ...
    try {
      const res = await api.post("/applicant/applications", {
        ...
        passport_issuing_authority: form.passport_issuing_authority || undefined,
        country_of_birth: form.country_of_birth || undefined,
      });
      setApplication(res.data.application);
      saveDraft(form, 2, res.data.application.id?.toString());
  ...
  const verifyPassport = async () => {
    if (!form.passport_number || !form.nationality || !form.passport_issue_date || !form.passport_expiry || !form.passport_issuing_authority) {
      toast.error("Please fill in all passport fields including issuing authority before verifying");
      return;
    }

    setPassportVerifying(true);
    try {
      const response = await api.post('/applicant/verify-passport', {
        passport_number: form.passport_number,
        nationality: form.nationality,
        issue_date: form.passport_issue_date,
        expiry_date: form.passport_expiry,
        issuing_authority: form.passport_issuing_authority,
      });

      setPassportVerificationResult(response.data);
      
      if (response.data.valid) {
        toast.success(response.data.message || "Passport verified successfully", { icon: "✅" });
      } else {
        toast.error(response.data.message || "Passport verification failed", { icon: "⚠️" });
      }
      ...
      toast.error(error.response?.data?.message || "Failed to verify passport");
      setPassportVerificationResult({ valid: false, message: "Verification failed" });
    } finally {
  ...
  const validate = (step: number): boolean => {
    ...
    if (step === 1) {
      ...
      if (!form.passport_issuing_authority.trim()) e.passport_issuing_authority = "Issuing authority is required";
      else {
  ...
  return (
    <DashboardShell title="New Visa Application" description="Complete each section to submit your application">
      ...
      {/* Dynamic Pricing Display - Only shows on Review & Pay step */}
          ...
          <div>
            ...
            <div className="mb-8">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <Select label="Nationality *" value={form.nationality} onChange={(e) => handleNationalityChange(e.target.value)} error={errors.nationality} required>
            ...
            <div className="mb-8">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <Input label="Passport Number *" value={form.passport_number} onChange={(e) => {
                  set("passport_number", e.target.value.toUpperCase());
                  setPassportVerificationResult(null);
                }} error={errors.passport_number} placeholder="e.g. AB1234567" required />
                <CustomSelect 
                  label="Issuing Authority *" 
                  value={form.passport_issuing_authority} 
                  onChange={(value) => {
                    set("passport_issuing_authority", value);
                    setPassportVerificationResult(null);
                  }}
                  options={PASSPORT_AUTHORITIES.map(auth => ({
                    value: auth.authority,
                    label: auth.authority,
                    searchText: `${auth.country} ${auth.authority}`
                  }))}
                  placeholder="Select issuing authority"
                  error={errors.passport_issuing_authority}
                  searchable
                  required 
                />
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <Input label="Issue Date *" type="date" value={form.passport_issue_date} onChange={(e) => {
                  set("passport_issue_date", e.target.value);
                  setPassportVerificationResult(null);
                }} error={errors.passport_issue_date} required />
                <Input label="Expiry Date *" type="date" value={form.passport_expiry} onChange={(e) => {
                  set("passport_expiry", e.target.value);
                  setPassportVerificationResult(null);
                }} error={errors.passport_expiry} hint="Must be valid 6+ months" required />
              </div>
              ...
              <div className="mt-4">
                <Button
                  ...
                  loading={passportVerifying}
                  disabled={!form.passport_number || !form.nationality || !form.passport_issue_date || !form.passport_expiry || !form.passport_issuing_authority}
                  variant="secondary"
                  leftIcon={<ShieldCheck size={16} />}
                  className="w-full sm:w-auto"
                >
                  {passportVerifying ? "Verifying Passport..." : "Verify Passport"}
                </Button>

                {passportVerificationResult && (
                  <div className={`mt-3 p-4 rounded-xl border ${
                    passportVerificationResult.valid 
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <p className={`text-sm font-semibold ${
                          passportVerificationResult.valid ? "text-success" : "text-danger"
                        }`}>
                          {passportVerificationResult.valid ? "Passport Verified" : "Verification Failed"}
                        </p>
                        <p className="text-xs text-text-muted mt-1">
                          {passportVerificationResult.message}
                        </p>
                        {passportVerificationResult.status && (
                          <p className="text-xs text-text-muted mt-1">
                            Status: {passportVerificationResult.status.replace(/_/g, " ").toUpperCase()}
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="bg-surface rounded-xl p-5">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <div><p className="text-xs text-text-muted">Issuing Authority</p><p className="text-sm font-medium text-text-primary">{form.passport_issuing_authority}</p></div>
